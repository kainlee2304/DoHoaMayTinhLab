...
int shadingMode = 2; // 0: Flat, 1: Gouraud, 2: Phong (mo phong)
...
void setupLighting() {
    ...
    GLfloat light_diffuse[] = { 0.8f, 0.8f, 0.8f, 1.0f }; // Anh sang khuyet tan
...
void display() {
    ...
    switch (shadingMode) {
        ...
        case 1:
            ...
            break;
        case 2:
            glShadeModel(GL_SMOOTH); // To bong theo che do Phong (mo phong)
            // De mo phong shading Phong, tang cuong specular highlight
            GLfloat enhanced_specular[] = { 2.0f, 2.0f, 2.0f, 1.0f };
            glMaterialfv(GL_FRONT, GL_SPECULAR, enhanced_specular);
            break;
    }
...
void keyboard(unsigned char key, int x, int y) {
    switch (key) {
        ...
        case 'm':
            shadingMode = (shadingMode + 1) % 3; // Chuyen doi giua cac che do to bong
            std::cout << "Shading mode: " << (shadingMode == 0 ? "Flat" : (shadingMode == 1 ? "Gouraud" : "Phong (simulated)")) << std::endl;
            break;
...
void printInstructions() {
    ...
    std::cout << "- Phim m: Chuyen doi giua cac che do to bong (Flat, Gouraud, Phong mo phong)\n";
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Minh hoa to bong noi suy Phong (mo phong)"); // Tao cua so moi
