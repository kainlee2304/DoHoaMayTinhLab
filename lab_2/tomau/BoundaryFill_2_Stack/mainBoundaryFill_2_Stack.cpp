...
// Cau truc du lieu LIST de luu toa do diem va cai dat ngan xep
struct LIST {
	int x, y;                // Toa do diem (x, y)
	struct LIST* next;       // Con tro toi phan tu tiep theo trong danh sach
};

struct LIST* top = NULL;     // Con tro tro den dinh cua ngan xep

// Ham day phan tu vao ngan xep
void push(int x, int y) {
    struct LIST* point;                                // Khai bao bien point kieu LIST
    ...
    if (top == NULL) {
        top = point;           // Neu ngan xep rong, diem nay la dinh moi
    } else {
        point->next = top;     // Neu khong, gan diem moi len tren dinh cua ngan xep
        top = point;           
    }
}
...
void pop(int *x, int *y) {
    struct LIST* point;
    point = top;              // Gan point bang dinh cua ngan xep
    top = top->next;          // Cap nhat dinh cua ngan xep
...
void BoundaryFill(int x, int y, float* fillColor, float* bc) {
    ...
    if ((color[0] != bc[0] || color[1] != bc[1] || color[2] != bc[2]) && 
        ...
        push(x, y);              // Day diem hien tai vao ngan xep
    }
}

// Ham Boundary Fill su dung ngan xep de tranh de quy
void BoundaryFill_Stack(int x0, int y0, float* fillColor, float* bc) {
    ...
    push(x0, y0);             // Day diem ban dau vao ngan xep
    while (top != NULL) {     // Lap lai cho den khi ngan xep rong
        pop(&x, &y);          // Lay diem tu dinh cua ngan xep
